...
class PessoaFisicaBusiness{
    ...
    public function selectByIdPessoaFisica($idPessoaFisica){
        ...
        INNER JOIN tbl_endereco AS e ON pfe.id_endereco = e.id_endereco WHERE pf.id_p_fisica = '$idPessoaFisica'";
        ...
        if($rsPF = $select->fetch(PDO::FETCH_ASSOC)){
            ...
            $result = array(
                ...
                'id_endereco' => $rsPF['id_endereco'],
                'status' => $rsPF['status'],
                'logradouro' => utf8_encode($rsPF['logradouro']),
            ...
            $result = array(
                'success' => false,
                'message' => 'Não foi possível resgatar os daddos'
            );
        }
        // Fechando a conexão com BD
        $this->conexao->closeDatabase();
